...
def assemble_line(line, label_table, current_address):
    ...
    # Immediate Arith: cmbi, mnsi, mlti, dvdi (op rs imm rd)
        ...
        print(rs)
        print(len(rs))
        imm = parse_immediate(operands[1], 16) # 15 bits for immediate
        print(imm)
        print(len(imm))
        rd = parse_register(operands[2])
        len((rd))
...
if __name__ == "__main__":
    if len(sys.argv)>1:
        ...
        input_file = "/mnt/c/code/CS 240/240FinalProject/Assembler/program1.mpsm"
        output_file = "/mnt/c/code/CS 240/240FinalProject/Assembler/program1.bin"
