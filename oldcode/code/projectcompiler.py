...
def evaluatingCondition(line, methods, indent):
    ...
    if("%" in line):
        ...
        num2 = num2[0]
        output+=f"{indentSpace}cmbi %r{rRegister},{num1}, %{rRegister}\n"
        rRegister1 = rRegister+1
        rRegister2 = rRegister+2
        output+=f"{indentSpace}mdlo %r{vars['iteration']},%r{rRegister}, %r{rRegister1}\n"
        
        output+=f"{indentSpace}ldim {num2},%r{rRegister}, %r{rRegister2}\n"
        for method in methods:
            output+=f"{indentSpace}ife %r{rRegister1},%{rRegister2},{method}\n"
